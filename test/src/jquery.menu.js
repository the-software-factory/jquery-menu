...
describe("jQuery-Menu functional tests", function() {
    ...
    // Tests that are time-dependent (animations testing with timeouts)
    describe("Async tests", function() {

        // Delay ( in milliseconds) to anticipate or wait to perform the matching control
        var timeoutOffset = 10;

        describe("Async tests for the menu with default options", function() {
            var timeout = $.fn.menu.defaults.delay;

            // If the mouse leaves the menu item, its submenu must be hiden after timeout
            ...
            // If the mouse leaves the menu item, its submenu must remain visible until timeout expires
            it("Doesn't hide before timeout", function(done) {
                defaultItems.firstMenuItem.mouseenter();
                defaultItems.firstMenuItem.mouseleave();
            ...
            // If the mouse leaves the menuitem but comes back within delay, its submenu must remain visible
            it("Is visible on mouse return before timeout", function(done) {
                defaultItems.firstMenuItem.mouseenter();
                defaultItems.firstMenuItem.mouseleave();
                ...
                setTimeout(function () {
                    expect(defaultItems.firstMenuItem).toBeVisible();
                    done();
                }, timeout + timeoutOffset);
            });

            // If the mouse leaves the menuitem but comes back within delay, its submenu must still have the visibleMenuItemClass CSS class attached
            it("Has visible menu class on mouse return before timeout", function(done) {
                defaultItems.firstMenuItem.mouseenter();
                defaultItems.firstMenuItem.mouseleave();
        ...
        describe("Async tests for the menu with custom options", function() {

            // If the mouse leaves the menu item, its submenu must be hidden after timeout
            it("Hides after timeout", function(done) {
                customItems.firstMenuItem.mouseenter();
                customItems.firstMenuItem.mouseleave();
                ...
                }, customOptions.delay + timeoutOffset);
            });
            ...
            // If the mouse leaves the menuitem but comes back within delay, its submenu must still have
            // the visibleMenuItemClass CSS class attached
            it("Has visible menu class on mouse return before timeout", function(done) {
                customItems.firstMenuItem.mouseenter();
                customItems.firstMenuItem.mouseleave();
